FUNCTION_FZF_LOCATION="$HOME/.fzf-functions.zsh"


#List FZF Functions
function fzf-function-list() {
if [[ -f $FUNCTION_FZF_LOCATION ]]; then
    selected=$(grep -E "(function fzf-)(.*?)[^(]*" $FUNCTION_FZF_LOCATION | sed -e "s/function fzf-//" | sed -e "s/() {//" | grep -v "selected=" | fzf --reverse --prompt="fzf functions > ")
else
    echo "fzf functions not found"
fi
    case "$selected" in
